...
# Usage:
python prepare_data_mlm.py --ds cola [replicates prepare_data.py]
python prepare_data_mlm.py --ds sst [replicates prepare_data.py]
python prepare_data_mlm.py --ds wiki
...
wikiSimplePilot = True	#use smaller pilot dataset
...
if cfg_ds=='sst':
	...
	from get_wiki_data import DataReader
	if(wikiSimplePilot):
		train, valid, test = DataReader("data/wikisimple/train-pilot.txt","./data/wikisimple/dev-pilot.txt","./data/wikisimple/test-pilot.txt").read()
	else:
		train, valid, test = DataReader("data/wikisimple/train.txt","./data/wikisimple/dev.txt","./data/wikisimple/test.txt").read()
	random.shuffle(train)
	random.shuffle(valid)
	random.shuffle(test)
	train_size=len(train)
	valid_size=len(valid)
	test_size=len(test)
	print('wiki (MLM), train_size:%d, valid_size:%d, test_size:%d'%(train_size,valid_size,test_size))
	trainset = {}
	validset = {}
	testset = {}
	BertForMaskedLMlabelFakeDiscarded = 0
	for data, dataset in [(train, trainset), (valid, validset), (test, testset)]:
		label = []
		all_text = []
		for line in data:
			#print("line = ", line)
			label.append(BertForMaskedLMlabelFakeDiscarded)
	...
	corpus_size=len(corpus)
	print("corpus_size = ", corpus_size)
	y_prob=np.eye(corpus_size,len(label2idx))
	print("y_prob.shape = ", y_prob.shape)
