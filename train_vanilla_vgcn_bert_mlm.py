...
"""train_vanilla_vgcn_bert_mlm.py
...
# Usage:
python train_vanilla_vgcn_bert_mlm.py --ds cola [replicates train_vanilla_vgcn_bert.py]
python train_vanilla_vgcn_bert_mlm.py --ds sst [replicates train_vanilla_vgcn_bert.py]
python train_vanilla_vgcn_bert_mlm.py --ds wiki

# Description:
vgcn_bert_mlm is designed to enable a bert transformer Masked Language Model (MLM) 
to perform masked/next word prediction based on its own previously/pretrained 
embeddings and the previous textual context as instantaneously stored in a Vocabulary 
Graph Convolutional Network (VGCN): for example all the previous sentences/paragraphs 
encountered in a book being read.
...
debugTrainFewEpochs = True	#use low epochs for debug
...
dataset_list={'sst', 'cola', 'wiki'}
# hate: 10k, mr: 6753, sst: 7792, r8: 5211

if(debugTrainFewEpochs):
...
if cfg_ds=='sst':
	...
	cfg_bertModelType = 'BertForSequenceClassification'
elif cfg_ds=='cola':
	batch_size = 8 #16 #12
	learning_rate0 = 8e-6 #2e-5  
	l2_decay = 0.01 
	cfg_bertModelType = 'BertForSequenceClassification'
elif cfg_ds=='wiki':	#mlm
	batch_size = 2 #16 #12	#low batch_size is required for long sentences (200 words)
	learning_rate0 = 8e-6 #2e-5  
	l2_decay = 0.01 
	cfg_bertModelType = 'BertForMaskedLM'
	
MAX_SEQ_LENGTH = 200
sequenceLengthAvgEst = 50
gcn_embedding_dim=1
...
#print("gcn_vocab_size = ", gcn_vocab_size)	#gcn_vocab_size =  12834
...
if(cfg_bertModelType == 'BertForMaskedLM'):
	total_train_steps = int(len(train_dataloader) / gradient_accumulation_steps * sequenceLengthAvgEst * total_train_epochs)
else:
...
def getIgnoreTokenID():
	#README.md: All labels set to -1 are ignored (masked)	#https://github.com/huggingface/transformers/tree/b832d5bb8a6dfc5965015b828e577677eace601e#2-bertforpretraining	#OLD: -100 
	ignore_index = -1
	return ignore_index

def getMaskedTokenID():
	#FUTURE: tokenizer.mask_token_id / 103	#https://github.com/huggingface/transformers/blob/b832d5bb8a6dfc5965015b828e577677eace601e/pytorch_pretrained_bert/tokenization.py#L74
	masked_tokenList = []
	masked_token = "[MASK]"
	masked_tokenList.append(masked_token)
	mask_token_idList = tokenizer.convert_tokens_to_ids(masked_tokenList)
	mask_token_id = mask_token_idList[0]
...
def evaluate(model, gcn_adj_list,predict_dataloader, batch_size, epoch_th, dataset_name):
	...
	with torch.no_grad():
		for batch in predict_dataloader:
			...
			if(cfg_bertModelType == 'BertForMaskedLM'):
				sequenceLength = input_mask.shape[1]
				prediction_ids = copy.deepcopy(input_ids)
				for maskIndex in range(1, sequenceLength):	#1: ignore start token
					
					masked_input_ids, masked_lm_labels = model.generateMaskedInputIdsAndLabels(input_ids, maskIndex)
					prediction_scoresAtMaskIndex = model(gcn_adj_list, gcn_swop_eye, masked_input_ids, segment_ids, input_mask, masked_lm_labels=None, maskIndex=maskIndex)	#loss, prediction_scores = 
					prediction_id = model.calculateBestPrediction(prediction_scoresAtMaskIndex, maskIndex=0)
					#prediction_ids[:, maskIndex] = model.calculateBestPrediction(prediction_scores, maskIndex)

					#replicated from model_vanilla_vgcn_bert_mlm.py;
					input_id = input_ids[:, maskIndex]
					masked_lm_loss = model.calculateLoss(prediction_scoresAtMaskIndex, masked_lm_labels, maskIndex)
					ev_loss+=masked_lm_loss.item()
					
					predict_out.extend(prediction_id.tolist())
					all_label_ids.extend(input_id.tolist())
					#eval_accuracy = model.calculatePredictionAccuracy(input_ids, prediction_ids)
					eval_accuracy = model.calculatePredictionAccuracyAtMaskIndex(input_id, prediction_id)
					total += len(label_ids)
					correct += eval_accuracy
								
			elif(cfg_bertModelType == 'BertForSequenceClassification'):
		...
		if(cfg_bertModelType == 'BertForSequenceClassification'):
...
#%%
if(cfg_bertModelType == 'BertForMaskedLM'):
	from model_vanilla_vgcn_bert_mlm import Vanilla_VGCN_Bert_mlm as Vanilla_VGCN_Bert
elif(cfg_bertModelType == 'BertForSequenceClassification'):
...
if(cfg_bertModelType == 'BertForMaskedLM'):
	mask_token_id = getMaskedTokenID()
	ignore_token_id = getIgnoreTokenID()
	model.initialiseTokenIds(mask_token_id=mask_token_id, ignore_token_id=ignore_token_id)
	...
	for step, batch in enumerate(train_dataloader):
		...
		if(cfg_bertModelType == 'BertForMaskedLM'):
			...
			for maskIndex in range(1, sequenceLength):	#1: ignore start token
				...
				masked_input_ids, masked_lm_labels = model.generateMaskedInputIdsAndLabels(input_ids, maskIndex)				
				loss = model(gcn_adj_list, gcn_swop_eye, masked_input_ids, segment_ids, input_mask, masked_lm_labels, maskIndex)
	
				#CHECKTHIS: gradient increment step size
